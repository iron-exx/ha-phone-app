...
- `ios-app/project.yml` defines an XcodeGen spec for the `HAPhoneTestApp` app target (bundle id `de.haphone.app.test`, `UIBackgroundModes: [voip]`, `aps-environment: development` entitlement) and `HAPhoneTestAppTests` unit test target, so the entire Xcode project can be generated from text without ever needing Xcode in this sandbox.
