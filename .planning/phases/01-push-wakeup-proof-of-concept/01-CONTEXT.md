...
- **D-10 (SUPERSEDED by D-11 — see below):** ~~iOS is built via GitHub Actions macOS runner, signed with a paid Apple Developer account, distributed via TestFlight.~~ Originally assumed a paid Apple Developer Program membership. Replaced by D-11 after the user confirmed a strict zero-budget constraint for this phase.

### Zero-Budget Constraint (supersedes D-10)
- **D-11:** The user will not pay for anything — no Apple Developer Program ($99/yr), no Google Play Console ($25 one-time). This is a hard constraint for Phase 1, not a preference. Consequence for iOS: a free "Personal Team" Apple ID cannot receive the Push Notifications entitlement under any circumstance (Apple gates `aps-environment` behind paid Program membership regardless of publishing intent or TestFlight-vs-App-Store distribution) — so **real VoIP push on a physical iPhone cannot be verified in Phase 1** at all. What CAN be done for free: build the iOS app via a GitHub Actions macOS runner (unsigned, no Apple ID needed), run `xcodebuild build`/`xcodebuild test` for the iOS Simulator, and verify the CallKit/PushKit wiring structurally (unit tests + a debug-triggered simulated incoming call in the Simulator, proving `reportNewIncomingCall` fires correctly) — but NOT real backgrounded/terminated/locked-device push delivery. Plan 04 must be rescoped to Simulator-only CI (no Fastlane match/TestFlight/ASC API key/code signing). Plan 06 must explicitly document PUSH-01/PUSH-02's real-iOS-device verification as an accepted, unresolved gap in the phase sign-off — not silently marked done.
- **D-12:** For Android, skip the Play Console "calling app" declaration entirely (it requires a $25 Google Play Developer account). Firebase project creation itself is free and entirely separate from Play Console/Play Store — proceed with that. Sideload the debug APK directly via `adb install` (already an existing fallback noted in Plan 05) rather than through Play Console's internal testing track. Whether the Android 14+ full-screen-intent auto-grant actually works for a sideloaded app that registers a self-managed ConnectionService/PhoneAccount (without any Play Console declaration) is unverified — test this empirically on the real Pixel and record the actual result (works / doesn't work) as a Phase 1 finding, rather than assuming either outcome.
- **Tension with PROJECT.md:** PROJECT.md's Constraints section currently states the Apple Developer Membership "fällt ohnehin an" (will be needed anyway) for eventual App Store publishing to serve other HA-Phone operators. D-11 does not change that long-term plan — it only defers the cost to whenever the user decides to move toward real publishing/distribution, which is out of scope for Phase 1's proof-of-concept.
