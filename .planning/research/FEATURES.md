...
| Door-station video preview before answering | Matches dedicated intercom apps (Akuvox SmartPlus, DoorBird, 2N) and Yeastar Linkus, which also previews door-station video while ringing | HIGH | Implemented via SIP early media (183 + H.264), like the Fanvil desk phone; Asterisk forwards early media only to a single dialed destination |
...
| Always reachable without a VPN in the home network | Replaces the status quo (Tailscale VPN + always-on Linphone) | HIGH | Permanent TLS registration kept alive by a foreground service (user decision 2026-09-23); push as fallback |
